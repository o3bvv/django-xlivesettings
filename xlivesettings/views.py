...
from xlivesettings import ConfigurationSettings, forms
from xlivesettings.overrides import get_overrides

log = logging.getLogger(__name__)

@csrf_protect
def group_settings(request, group, template='xlivesettings/group_settings.html'):
                ...
                for name, value in form.cleaned_data.items():
                    ...
                    if cfg.update(value):
                        ...
                        messages.add_message(
                            ...
                            _("Updated %(key)s on %(grp)s") % {'key': cfg.key, 'grp': cfg.group.key})
...
group_settings = never_cache(permission_required('xlivesettings.change_setting')(group_settings))
...
def site_settings(request):
    return group_settings(request, group=None, template='xlivesettings/site_settings.html')

def export_as_python(request):
    """Export site settings as a dictionary of dictionaries"""

    from xlivesettings.models import Setting, LongSetting
    import pprint
    ...
    return render_to_response('xlivesettings/text.txt', { 'text' : pretty }, mimetype='text/plain')
